    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(csrf -> csrf.disable()) // CSRF 비활성화
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/auth/login", "/auth/signup","/api/users/signup", "/swagger-ui/**", "/v3/api-docs/**", "/h2-console/**","/api/auth/email/verify", "/api/auth/email/confirm","/api/users/delete/**","/api/auth/email/password/verify","/api/auth/email/password/reset", "/ws/**").permitAll() // 인증 없이 접근 허용
